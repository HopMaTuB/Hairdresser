...
async def signup(body: UserModel, background_tasks: BackgroundTasks, request: Request, db: Session = Depends(get_db)):
    exist_user = UserRepository.get_user_by_email(body.email, db)
    if exist_user:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Account already exists")
    body.password = auth_service.get_password_hash(body.password)
    new_user = UserRepository.create_new_user(body, db)
...
async def login(request: Request, body: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    user = UserRepository.get_user_by_email(body.username, db)
    if user is None:
...
async def confirmed_email(token: str, db: Session = Depends(get_db)):
    ...
    UserRepository.confirmed_email(email, db)
...
async def request_email(body: RequestEmail, background_tasks: BackgroundTasks, request: Request,
    ...
    user = UserRepository.get_user_by_email(body.email, db)
